...
def parse_args():
    ...
    parser.add_argument(
        "--n_boids", type=int, default=20, help="Number of balls in the simulation."
    )
    parser.add_argument(
        "--r_o", type=float, default=5, help="radius of orientation."
    )
    
    parser.add_argument("--seed", type=int, default=42, help="Random seed.")
    parser.add_argument("--ver", type=int, default=42, help="version")
    parser.add_argument(
        "--datadir", type=str, default="boid", help="Name of directory to save data to."
    )
...
def generate_dataset(args, width, sim, num_sims, length, sample_freq, train=1):
    ...
    suffix += "_ro" + str(args.r_o)

    if args.ver > 0:
        suffix += "_" + str(args.ver)
    ...
    print(suffix)
    if not os.path.exists(args.datadir+'/'+suffix):
        os.makedirs(args.datadir+'/'+suffix)
    if not os.path.exists(args.datadir+'/video/'+suffix):
        os.makedirs(args.datadir+'/video/'+suffix)
    ...
    for i in range(num_sims):
        sim_i = sim
        t = time.time()
        loc, vel, phi, edges, treatment = sim_i.sample_trajectory(
            args,
            i,
            T=length,
        ...
        data = [loc, phi, vel, treatment, edges]
        np.save(os.path.join(args.datadir+'/'+suffix, str(i)+".npy"), data)

        # angular velocitysuffix
        #center = np.mean(loc,2)
        #vec_ic = loc - center[:,:,np.newaxis].repeat(args.n_boids,2)
        #angvel = np.mean(np.cross(vec_ic,vel,axis=1))

        if True: # i % 100 == 0:
            print("Iter: {}, Simulation time: {:.1f}".format(i, time.time() - t))
            # print("Iter: {}, Simulation time: {:.1f}, Ang vel: {:.3f}".format(i, time.time() - t, angvel))
            
        if i < args.video and train == 1:
            '''loc_all.append(loc)
            vel_all.append(vel)
            edges_all.append(edges)
            edges_res_all.append(edges_result)
            treatment_all.append(treatment)'''
            for s in range(loc.shape[0]):
                vel_norm = np.sqrt((vel[s] ** 2).sum(axis=1))
                loc_ = loc[s]
                edge = edges[s] # edges_result[s]
                try: fig = plt.figure()
                except: import pdb; pdb.set_trace()
                axes = plt.gca()
                axes.set_xlim([-width, width])
                axes.set_ylim([-width, width])
                for t in range(loc_.shape[-1]):
                    plt.plot(loc_[:, 0, t], loc_[:, 1, t])
                    plt.plot(loc_[0, 0, t], loc_[0, 1, t], "d")
                ...
                lines = [
                    plt.plot([], [], marker="$" + "{:d}".format(t) + "$", alpha=1, markersize=10)[0]
                    for t in range(loc_.shape[-1])
                ]

                ani = FuncAnimation(fig, update, frames=loc_.shape[0], fargs = (lines,loc_,edge),interval=5, blit=True)

                try: ani.save(filename=os.path.join(args.datadir+'/video/'+suffix, str(i)+"_"+str(s)+".mp4"), writer=writer)
                except: import pdb; pdb.set_trace()
    ...
    return loc_all, vel_all, edges_res_all, edges_all, treatment_all
...
if __name__ == "__main__":
    ...
    if args.simulation == "boid":
        width = 50 # 15
        sim = Boid(args.n_boids, width, 1, r_orientation=args.r_o, noise_var=0.01)
    ...
    if True:
        ...
        args.train=1
        loc_train, vel_train, edges_res_train, edges_train, treatments_train  = generate_dataset(
            args, width, sim,
            args.num_train, 
            T,
    ...
    if True:
        ...
        args.train = 0
        loc_valid, vel_valid, edges_res_valid, edges_valid, treatment_valid = generate_dataset(
            args, width, sim,
            args.num_valid,
            T,
            args.sample_freq, train=args.train, 
    ...
    args.train = -1
    loc_test, vel_test, edges_res_test, edges_test, treatment_test = generate_dataset(
        args, width, sim,
        args.num_test,
        T,
        args.sample_freq, train=args.train,
